...
)
{
	...
	if (nArgc == 1)
	{
		...
		printf("%16s %16s %16s %16s %24s %32s %s\n", "ID", "IP", "STATE", "SWITCH ID", "Registered Time", "Hash", "Comment");

		xRet = FTM_CATCHB_getCCTVList(pCatchB, 10, pCCTVList, &ulCount);
		for(i = 0 ; i < ulCount ; i++)
		{
			printf("%16s %16s %16s %16s %24s %32s %s\n", 
					pCCTVList[i].xConfig.pID, 
					pCCTVList[i].xConfig.pIP, 
					FTM_CCTV_STAT_print(pCCTVList[i].xConfig.xStat), 
					pCCTVList[i].xConfig.pSwitchID, 
					FTM_TIME_printf2(pCCTVList[i].xConfig.ulTime, NULL), 
					pCCTVList[i].xConfig.pHash,
					pCCTVList[i].xConfig.pComment);
		}
	}
