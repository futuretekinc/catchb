...
)
{
	...
	{
		xRet = FTM_SHELL_CMD_showAlarmList(pClient);
	}
	...
	{
		xRet = FTM_CLIENT_delAlarm(pClient, pArgv[2]);
		if (xRet != FTM_RET_OK)
		{
	...
	else  if (strcasecmp(pArgv[1], "get") == 0)
	{
		...
		xRet = FTM_CLIENT_getAlarm(pClient, pArgv[2], &xAlarm);
		if (xRet == FTM_RET_OK)
		{
			printf("%8s : %s\n", "Name", 	xAlarm.pName);
	...
	else  if (strcasecmp(pArgv[1], "set") == 0)
	{
		...
		else
		{
			...
			xRet = FTM_CLIENT_setAlarm(pClient, pArgv[2], &xAlarm, ulFieldFlags);
			if (xRet != FTM_RET_OK)
			{
...
)
{
	...
	pNameList = (FTM_NAME_PTR)FTM_MEM_calloc(sizeof(FTM_NAME), ulCount);
	if (pNameList == NULL)
	{
	...
	else
	{
		...
		for(i = 0; i < ulCount ; i++)
		{
			...
			xRet = FTM_CLIENT_getAlarm(pClient, pNameList[i], &xAlarm);
			if (xRet == FTM_RET_OK)
			{
				printf("%4d : %16s %24s %s\n", i+1, xAlarm.pName, xAlarm.pEmail, xAlarm.pMessage);
			}
		}
	}

	FTM_MEM_free(pNameList);
