...
)
{
	...
	ulFormatLen += snprintf(&pFormat[ulFormatLen], sizeof(pFormat) - ulFormatLen - 1, "_IDX=%d _TARGET=%s _LOCAL=%s acl.sh %s", ulIndex, pTargetIP, pLocalIP, pFileName);
