...
int db_cctv_info_select(int * t_num)
{
    ...
    if (connection == NULL)
    {
       cctv_system_error("cctv_scheduler/db_cctv_info_select() - Mysql connection error : %s",strerror(errno));
        return 1;
    }

    query_stat = mysql_query(connection, "select * from tb_ck_ip_info");
    if (query_stat != 0)
    {
       mysql_close(connection);
       cctv_system_error("cctv_scheduler/db_cctv_info_select() - Mysql query error: %s",strerror(errno));
        return 1;
    }
    ...
    while((sql_row = mysql_fetch_row(sql_result)) != NULL)
    {
        if(ip_check(sql_row[2]) == false){
            ...
            check_ip_info->ck_cctv_idx = safe_atoi(sql_row[0]);

            memcpy(check_ip_info->ck_cctv_id, sql_row[1], sizeof(check_ip_info->ck_cctv_id));

            memcpy(check_ip_info->ck_ip, sql_row[2], sizeof(check_ip_info->ck_ip));

            //log_message(CK_CCTV_SCHEDULER_LOG_FILE_PATH, "select analysis cctv idx:%d, cctv id :%s, cctv_ip:%s\n",sql_row[0],sql_row[1], sql_row[2]);

            printf("[%s][%d] cctv idx:%d, cctv id :%s, cctv_ip:%s\n",__func__,__LINE__,check_ip_info->ck_cctv_idx, check_ip_info->ck_cctv_id, check_ip_info->ck_ip);
            //check_ip_info->ck_status = 0;
...
int db_cctv_info_delete()
{
    ...
    if (connection == NULL)
    {
        cctv_system_error("cctv_scheduler/db_cctv_info_delete - Mysql connection error : %s",strerror(errno));
        return 1;
    }

    sprintf(query, "delete from tb_ck_result_info");

    query_stat = mysql_query(connection, query);
    if (query_stat != 0)
    {
        mysql_close(connection);
        cctv_system_error("cctv_scheduler/db_cctv_info_delete() - Mysql query error: %s",strerror(errno));
        return 1;
    }
...
int db_cctv_info_insert(int cctv_idx, char *cctv_id, char *ip, char *hash_value, int code)
{
    ...
    if (connection == NULL)
    {
        cctv_system_error("cctv_scheduler/db_cctv_log_insert() - Mysql connection error : %s",strerror(errno));
        return 1;
    }
    ...
    sprintf(query, "insert into tb_ck_result_info(C_CCTV_IDX, C_CCTV_ID,C_CCTV_IP,C_CCTV_SIGNATURE,CODE) values(%d,'%s','%s','%s',%d);", cctv_idx, cctv_id, ip, hash_value,code);

    query_stat = mysql_query(connection, query);
    if (query_stat != 0)
    {
        mysql_close(connection);
        cctv_system_error("cctv_scheduler/db_cctv_info_insert() - Mysql query error: %s",strerror(errno));
        return 1;
    }
...
void send_http_request(int port_number, char *analysis_cctv_ip)
{
    ...
    {
        cctv_system_error("cctv_scheduler/send_http_request() - Error while creating the socket");
        return ;
    }
    ...
    if ((n = connect(sockfd, ((struct sockaddr*)(&serv_addr)), sizeof(serv_addr))) < 0)
    {
		...
		{
			cctv_system_error("cctv_scheduler/send_http_request() - Error while connecting");
			close(sockfd);
			return ;
		}
	}
	...
	{
		ERROR(-1, "Failed to set non block!");
		close(sockfd);
		return;
	}
...
void* thread_main_operation(void *id)
{
    ...
    list_for_each(pos, &check_ip_list_head){
        ...
        //log_message(CK_CCTV_SCHEDULER_LOG_FILE_PATH,"analysis setting ip :%s start",score_info.analysis_cctv_ip);
        //ping æ»æ≤∏È ø©±‚ forπÆ ∏∑æ∆¡÷∏Èµ»¥Ÿ
        for( i = 0; i< 3; i++)
		{
            ...
            
        {
            ...
            if(dev == NULL)
            {
                ...
                cctv_system_error("cctv_scheduler/thread_main_operation() - error not device");
                log_message(CK_CCTV_SCHEDULER_LOG_FILE_PATH, "cctv_scheduler/thread_main_operation() - error not device :%s",score_info.analysis_cctv_ip);
                sem_post(&sendthread);
                sem_wait(&mainthread);
                sleep(4);
            ...
            if(BUFSIZ < snap_leng){
                ...
                cctv_system_error("cctv_scheduler/thread_main_operation() -pcap_snapshot error");
                log_message(CK_CCTV_SCHEDULER_LOG_FILE_PATH, "cctv_scheduler/thread_main_operation() - error pcap_snapshot:%s",score_info.analysis_cctv_ip);
                continue;
            }
            ...
            if(pcap_lookupnet(dev, &net, &mask, errbuf) < 0){
                ...
                cctv_system_error("cctv_scheduler/thread_main_operation() -pcap_lookupnet error");
                log_message(CK_CCTV_SCHEDULER_LOG_FILE_PATH, "cctv_scheduler/thread_main_operation() - error pcap_lookupnet:%s",score_info.analysis_cctv_ip);
                continue;
            ...
            if(pcap_compile(score_info.descr, &fcode, filter_rule, 0, mask) < 0){
                ...
                cctv_system_error("cctv_scheduler/thread_main_operation() -pcap_compile error");
                log_message(CK_CCTV_SCHEDULER_LOG_FILE_PATH, "cctv_scheduler/thread_main_operation() - pcap_compile:%s",score_info.analysis_cctv_ip);
            ...
            if(pcap_setfilter(score_info.descr, &fcode) < 0 ){
                ...
                cctv_system_error("cctv_scheduler/thread_main_operation() -pcap_setfilter error");
                log_message(CK_CCTV_SCHEDULER_LOG_FILE_PATH, "cctv_scheduler/thread_main_operation() - error pcap_setfilter:%s",score_info.analysis_cctv_ip);
            ...
            //log_message(CK_CCTV_SCHEDULER_LOG_FILE_PATH, "ip:%s, string:%s",score_info.analysis_cctv_ip, hash_string);
            ...
            //db Í≤∞Í≥º Ï†ÄÏû•
            db_cctv_info_insert(cctv_idx, cctv_id, score_info.analysis_cctv_ip, hash_value,code);
...
{ 
    ...
    // ø¨∞·¿ª ±‚¥Ÿ∏∞¥Ÿ. 
    // Non blocking ªÛ≈¬¿Ãπ«∑Œ πŸ∑Œ ∏Æ≈œ«—¥Ÿ. 
    ...
    // ¡ÔΩ√ ø¨∞·¿Ã º∫∞¯«ﬂ¿ª ∞ÊøÏ º“ƒœ¿ª ø¯∑° ªÛ≈¬∑Œ µ«µπ∏Æ∞Ì ∏Æ≈œ«—¥Ÿ.  
    if (res == 0) 
    { 
